...
function AuthProviderContent({ children }: { children: ReactNode }) {
  ...
  const login = useCallback(async (email: string, password: string) => {
    try {
      setLoading(true);
      const response = await handleApiRequest(API_ENDPOINTS.AUTH.LOGIN, {
        method: 'POST',
        headers: {
  ...
  const register = useCallback(async (name: string, email: string, password: string) => {
    try {
      setLoading(true);
      const response = await handleApiRequest(API_ENDPOINTS.AUTH.REGISTER, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const forgotPassword = useCallback(async (email: string) => {
    try {
      setLoading(true);
      const response = await handleApiRequest(API_ENDPOINTS.AUTH.FORGOT_PASSWORD, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const resetPassword = useCallback(async (token: string, password: string) => {
    try {
      setLoading(true);
      const response = await handleApiRequest(`${API_ENDPOINTS.AUTH.RESET_PASSWORD}/${token}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
